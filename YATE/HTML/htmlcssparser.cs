...
namespace HTMLConverter
{
    ...
    internal static class HtmlCssParser
    {
        ...
        internal static void GetElementPropertiesFromCssAttributes(XmlElement htmlElement, string elementName, CssStylesheet stylesheet, Hashtable localProperties, List<XmlElement> sourceContext)
        {
            ...
            string styleInline = HTMLToFlowConverter.GetAttribute(htmlElement, "style");
            ...
            if (style != null)
            {
                ...
                for (int i = 0; i < styleValues.Length; i++)
                {
                    ...
                    if (styleNameValue.Length == 2)
                    {
                        ...
                        string styleValue = HTMLToFlowConverter.UnQuote(styleNameValue[1].Trim()).ToLower();
                        int nextIndex = 0;
                        ...
                        switch (styleName)
                        {
                            ...
                            case "font-family":
                                ...
                                break;
                            case "font-size":
                                 ParseCssSize(styleValue, ref nextIndex, localProperties, "font-size", /*mustBeNonNegative:*/true,10);
                                break;
                            case "font-style":
                            ...
                            case "vertical-align":
                                ...
                                break;
                            case "text-indent":
                                ParseCssSize(styleValue, ref nextIndex, localProperties, "text-indent", /*mustBeNonNegative:*/false, 0);
                                break;

                            case "width":
                            case "height":
                                ParseCssSize(styleValue, ref nextIndex, localProperties, styleName, /*mustBeNonNegative:*/true,1);
                                break;
                            ...
                            case "padding-bottom":
                            case "padding-left":
                                ParseCssSize(styleValue, ref nextIndex, localProperties, styleName, /*mustBeNonNegative:*/true,0);
                                break;
        ...
        private static double ParseCssDoubleSize(string styleValue, ref int nextIndex, bool mustBeNonNegative,double def)
        {
            ParseWhiteSpace(styleValue, ref nextIndex);
            ...
            if (nextIndex < styleValue.Length && Char.IsDigit(styleValue[nextIndex]))
            {
                ...
                string unit = ParseWordEnumeration(_fontSizeUnits, styleValue, ref nextIndex);
                Double r = TryGetLengthValue(number, unit);

                r = (r < 0 ? 0 : r);

                return (r == Double.NaN ? def : (double)r);
        ...
        private static void ParseCssSize(string styleValue, ref int nextIndex, Hashtable localValues, string propertyName, bool mustBeNonNegative,double def)
        {
            localValues[propertyName] = ParseCssDoubleSize(styleValue, ref nextIndex, mustBeNonNegative,def);
        }
        ...
        /// <returns></returns>
        private static Double TryGetLengthValue(string number, string unit)
        {
            ...
            if (number != null)
            {
                ...
                switch (unit)
                {
                    case "em":
                        {
                            if (Double.TryParse(number, out length))
                            {
                                length = 16* length; // convert from points to pixels
                            }
                            else
                            {
                                length = Double.NaN;
        ...
        private static Brush ParseCssColor(string styleValue, ref int nextIndex)
        {
            ...
            if (nextIndex < styleValue.Length)
            {
                ...
                if (character == '#')
                {
                    ...
                    while (startIndex < styleValue.Length)
                    {
                        character = Char.ToUpper(styleValue[startIndex]);
                        if (('0' <= character && character <= '9' || 'A' <= character && character <= 'F'))
                        {
                            break;
                        }
                ...
                else if (styleValue.Substring(nextIndex, 3).ToLower() == "rgb")
                {
                    ...
                    if (nextIndex > startIndex + 1) //r
                    {
                        Byte.TryParse(styleValue.Substring(startIndex, nextIndex - startIndex), out r);
                    ...
                    if (nextIndex > startIndex + 1) //g
                    {
                        Byte.TryParse(styleValue.Substring(startIndex, nextIndex - startIndex), out g);
                    ...
                    if (nextIndex > startIndex + 1) //g
                    {
                        Byte.TryParse(styleValue.Substring(startIndex, nextIndex - startIndex), out b);
                ...
                else if (Char.IsLetter(character))
                {
                    ...
                    if (color == null)
                    {
                        ...
                        if (color != null)
                        {
                            ...
                            //  color = "black";
                           // cr = System.Drawing.Color.FromName(color);
                        } 
                         else
                        {
                            color = styleValue;
                    ...
                    if (!_cssColorLowerSpace.TryGetValue(color.ToLower(),out br))
                    {
            ...
            return br;// new SolidColorBrush(Color.FromRgb(r,g,b));
        }
        ...
        private static readonly Dictionary<string, Brush> _cssColorLowerSpace = new Dictionary<string, Brush>
        {
{"activeborder",SystemColors.ActiveBorderBrush},
{"activecaption",SystemColors.ActiveCaptionBrush},
{"activecaptiontext",SystemColors.ActiveCaptionTextBrush},
{"appworkspace",SystemColors.AppWorkspaceBrush},
{"control",SystemColors.ControlBrush},
{"controldark",SystemColors.ControlDarkBrush},
{"controldarkdark",SystemColors.ControlDarkDarkBrush},
{"controllight",SystemColors.ControlLightBrush},
{"controllightlight",SystemColors.ControlLightLightBrush},
{"controltext",SystemColors.ControlTextBrush},
{"desktop",SystemColors.DesktopBrush},
{"graytext",SystemColors.GrayTextBrush},
{"highlight",SystemColors.HighlightBrush},
{"highlighttext",SystemColors.HighlightTextBrush},
{"hottrack",SystemColors.HotTrackBrush},
{"inactiveborder",SystemColors.InactiveBorderBrush},
{"inactivecaption",SystemColors.InactiveCaptionBrush},
{"inactivecaptiontext",SystemColors.InactiveCaptionTextBrush},
{"info",SystemColors.InfoBrush},
{"infotext",SystemColors.InfoTextBrush},
{"menu",SystemColors.MenuBrush},
{"menutext",SystemColors.MenuTextBrush},
{"scrollbar",SystemColors.ScrollBarBrush},
{"window",SystemColors.WindowBrush},
{"windowframe",SystemColors.WindowFrameBrush},
...
{"buttonhighlight",SystemColors.ControlLightBrush},
{"buttonshadow",SystemColors.ControlDarkBrush},
{"gradientactivecaption",SystemColors.GradientActiveCaptionBrush},
{"gradientinactivecaption",SystemColors.GradientInactiveCaptionBrush},
{"menubar",SystemColors.MenuBarBrush},
{"menuhighlight",SystemColors.MenuHighlightBrush}
        ...
        private static void ParseCssFont(string styleValue, Hashtable localProperties)
        {
            ...
            {
                nextIndex++;
                ParseCssSize(styleValue, ref nextIndex, localProperties, "line-height", /*mustBeNonNegative:*/true,10);
            }
        ...
        {
            FontStyle v;
            string ls = ((string)styleValue).ToLower();
            switch (ls)
            {
                case "normal":
                    v = FontStyles.Normal;
        ...
        private static void ParseCssFontWeight(string styleValue, ref int nextIndex, Hashtable localProperties)
        {
            ...
            switch (ls)
            {
                ...
                case "500":
                   v = FontWeights.Medium;

                    break;
                case "600":
                    v = FontWeights.DemiBold;
                ...
                case "800":
                    ...
                    break;
                case "900":
                    v = FontWeights.ExtraBlack;
                    break;
                default:
                    v = FontWeights.Normal;
                    break;
            }
            localProperties["font-weight"] = v;
        }
        ...
        private static void ParseCssFontFamily(string styleValue, ref int nextIndex, Hashtable localProperties)
        {
            ...
            {
                localProperties["font-family"] = new FontFamily(fontFamilyList);
            }
        }
        ...
        private static bool ParseCssRectangleProperty(string styleValue, ref int nextIndex, Hashtable localProperties, string propertyName)
        {
            ...
            {
               case "border-color":
                   value = ParseCssColor(styleValue, ref nextIndex);
                    break;
                case "border-style":
                    value = ParseCssBorderStyle(styleValue, ref nextIndex);
                    break;
                default:
                    value = ParseCssDoubleSize(styleValue, ref nextIndex, /*mustBeNonNegative:*/true, 0);
                    break;
            }
    ...
    internal class CssStylesheet
    {
        ...
        private bool MatchSelectorLevel(string selectorLevel, XmlElement xmlElement)
        {
            ...
            if (selectorId != null && HTMLToFlowConverter.GetAttribute(xmlElement, "id") != selectorId)
            {
                return false;
            }

            if (selectorClass != null && HTMLToFlowConverter.GetAttribute(xmlElement, "class") != selectorClass)
            {
                return false;
            }
